...
def network_independents(args):
    ...
    """9. operation distance and vehicles"""
    # 计算车均里程
    ...
    """10. 新增指标"""
    # 1. operation_distance/route_total_distance
    operation_to_total_ratio = total_distance_by_city / total_distance / 1000 if total_distance > 0 else 0

    # 2. 平均速度：vs_parking_df中distance（米）/duration（分钟）的平均值（km/h）
    # 速度 = 距离(米) / 时间(分钟) * 60(分钟/小时) / 1000(米/公里)
    vs_parking_df['speed_kmh'] = (vs_parking_df['distance'] / vs_parking_df['duration']) * 60 / 1000
    avg_speed_kmh = vs_parking_df['speed_kmh'].mean()

    # 3. 线路数/bus terminal比率
    route_count = vs_parking_df['route_id'].nunique()
    terminal_count = vs_parking_df['destination'].nunique()
    route_terminal_ratio = route_count / terminal_count if terminal_count > 0 else 0

    # 4. 拥有10条线以上的bus terminal占比
    routes_per_terminal = vs_parking_df.groupby('destination')['route_id'].nunique()
    terminals_with_10plus_routes = (routes_per_terminal >= 3).sum()
    terminals_with_10plus_routes_ratio = terminals_with_10plus_routes / terminal_count if terminal_count > 0 else 0

    # 5. 平均每条线路发车时间覆盖的小时数
    # 计算每条线路的发车时间跨度（小时）
    hours_per_route = vs_parking_df.groupby('route_id')['hour'].nunique()
    avg_coverage_hour = hours_per_route.mean()

    info_slice_new_metrics = [operation_to_total_ratio, avg_speed_kmh, route_terminal_ratio,
                              terminals_with_10plus_routes_ratio, avg_coverage_hour]
    ...
    return {
        ...
        # 新增指标
        'operation_total_distance_ratio': info_slice_new_metrics[0],
        'avg_speed': info_slice_new_metrics[1],
        'route_terminal_ratio': info_slice_new_metrics[2],
        'terminal_having_3_routes': info_slice_new_metrics[3],
        'avg_service_hour': info_slice_new_metrics[4]
    }
