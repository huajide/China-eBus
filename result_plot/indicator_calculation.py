...
def initialize_global_data():
    ...
    if ZONE_GDF is None:
        ZONE_GDF = gpd.read_file(r'data/geodata/County_UTM.shp')
...
def calculate_population_coverage(city_name, province_name, STOP_GDF):
    ...
    zone_gdf = ZONE_GDF
    city_level = ['Shanghai', 'Beijing', 'Chongqing', 'Tianjin']
    entity_city = ['Zhongshan', 'Dongguan', 'Jiayuguan', 'Danzhou']

    if province_name in city_level:
        sub_zone = zone_gdf[(zone_gdf['province'] == city_name) & (zone_gdf['county'] == 'district')].reset_index(drop=True)
    elif city_name in entity_city:
        sub_zone = zone_gdf[zone_gdf['city'] == city_name].reset_index(drop=True)
    else:
        sub_zone = zone_gdf[(zone_gdf['city'] == city_name) & (zone_gdf['county'] == 'district')].reset_index(drop=True)
    if len(sub_zone) == 0:
...
def network_independents(args):
    ...
    '0. Loading files'
    route_shp = gpd.read_file(f'../data/cnbusdata/{province_name}/{city_name}/{city_name}_route5.shp', crs=CRS.from_epsg(4326))
    stop_shp = gpd.read_file(f'../data/cnbusdata/{province_name}/{city_name}/{city_name}_stop5.shp', crs=CRS.from_epsg(4326))
