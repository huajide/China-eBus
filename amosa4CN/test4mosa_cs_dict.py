...
def generate_all_d2s_dict_for_city(city_province, sim_n=50):
    """Generate all_d2s_dict for a single city."""
    try:
        print(f"Processing city: {city_province}")

        # Read vehicle-trip data.
        vs_parking_df = pd.read_csv(f'../data/input/vs_parking_nodeid/{city_province}.csv')

        # Read charging-station data.
        cs_gdf = gpd.read_file(f'../data/input/cs_gdf/{city_province}.shp', crs=CRS.from_epsg(4547))

        # Read road-network data.
        nodes_sim = gpd.read_file(f'../data/road/{city_province}/nodes_sim.shp', crs=CRS.from_epsg(4547))
        edges_sim = gpd.read_file(f'../data/road/{city_province}/edges_sim.shp', crs=CRS.from_epsg(4547))
        ...
        with open(f"{output_dir}/{city_province}.pkl", 'wb') as f:
            pickle.dump(all_d2s_dict, f)

        print(f"Finished processing city: {city_province}")
        return f"City {city_province} processed successfully"

    except Exception as e:
        print(f"Error processing city {city_province}: {str(e)}")
        return f"City {city_province} failed: {str(e)}"
...
if __name__ == '__main__':
    ...
    all_cities = (cities_df['city'].astype(str) + '_' + cities_df['province'].astype(str)).tolist()
