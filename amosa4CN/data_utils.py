...
def derive_simV_info(df: pd.DataFrame):
    ...
    base_energy = df['base_energy'].to_list()
...
def find_near_cs(location: int, n: int, cs_gdf: gpd.GeoDataFrame, nodes_gdf: gpd.GeoDataFrame, is_projected=False) \
    ...
    if is_projected:
        d2s_df = pd.DataFrame(
            cs_gdf.apply(
                lambda x: np.sqrt((x.geometry.x - destination_lon) ** 2 + (x.geometry.y - destination_lat) ** 2),
                axis=1
            ),
            columns=['d2s']
        )
    else:
        d2s_df = pd.DataFrame(cs_gdf.apply(lambda x: haversine((x.geometry.x, x.geometry.x), destination_loc), axis=1),
                              columns=['d2s'])
...
def derive_vs_cs(sim_v_info: pd.DataFrame, all_d2s: pd.DataFrame, cs_gdf: gpd.GeoDataFrame,
    ...
    d2s_dict = {key: [] for key in destination_list}
    ...
    for d in destinations:
        ...
        for s, row in sim_cs.iterrows():
            if d in sim_cs_dict:
                ...
                # Optionally create a new entry if des_id is missing.
                sim_cs_dict[d] = [{'station': s, 'distance': row.distance}]

    return sim_cs_dict

def simplify_vs_df(vs_parking: pd.DataFrame,large_bat=100, medium_bat=80, small_bat=50):
    ...
    # Iterate over groups split by v_name.
    for v_name, group in vs_df.groupby('v_name'):
        group = group.reset_index(drop=True)
        vehicle_type = group.iloc[0]['vehicle_type']
        ...
        accumulated_base_energy, accumulated_energy = 0,0
        accumulated_distance, accumulated_duration = 0,0
        merged = 0
        i=0
        while i < len(group):
        # for i in range(len(group)):
            accumulated_base_energy += group.iloc[i]['base_energy']
            accumulated_energy += group.iloc[i]['base_energy']+1.2*group.iloc[i]['distance']/1000
            ...
            if merged or len(group)==1:
                ...
                if (group['s_time'][i+1]-group['e_time'][i]).total_seconds()/60 >= 60:
                    ...
                    i -=1
                    end_index = i
                    if accumulated_base_energy:
                        merged_row = group.iloc[end_index].copy()
                        merged_row['base_energy'] = accumulated_base_energy
                        ...
                        result_df = result_df._append(merged_row, ignore_index=True)
                    merged = 1
                elif accumulated_energy > max_bat:
                    accumulated_base_energy -= (group.iloc[i]['base_energy']+group.iloc[i-1]['base_energy'])
                    accumulated_energy -= ((group.iloc[i]['base_energy']+group.iloc[i-1]['base_energy'])
                                           +1.2*(group.iloc[i]['distance']+group.iloc[i-1]['distance'])/1000)
...
def get_edge_attr_dist(coords, edge_index):
    ...
    # Convert the distance matrix to edge weights.
...
# Find the nearest station index for each origin and destination.
...
def get_edge_attr_route(vs_parking0: pd.DataFrame, coords, num_nodes, edge_index):
    ...
    # Parse coordinate strings into float tuples.
    ...
    # Build a BallTree for nearest-neighbor queries.
...
if __name__ == '__main__':
    vs_parking_df = pd.read_csv(r'data/all_vs_parking_nodeid.csv')
    ...
    vs_parking_df_new = simplify_vs_df(vs_parking_df,100)
