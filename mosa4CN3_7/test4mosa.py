...
import data_utils
import sim_utils
from vehicle_type import VehicleTypes, battery_capacity
...
class Location(Problem):
    ...
    def eval_vars(self, vars_, is_test=False, **kwargs):
        ...
        for i in range(len(self.simv_v_name)):
            ...
            degraded_vehi.driving_range = degraded_vehi.driving_range * degradation_factor
        ...
        if use_custom_extra:
            ...
            saved_time, subs_idx, extra_large, extra_medium, extra_small = extra_timesave(
                self.v_name, self.distance, timeout_list, self.v_type,
                VehicleTypes(vars_[self.cs_num], v_type='large').driving_range*degradation_factor,
                VehicleTypes(vars_[self.cs_num + 1], v_type='medium').driving_range*degradation_factor,
                VehicleTypes(vars_[self.cs_num + 2], v_type='small').driving_range*degradation_factor,
...
if __name__ == '__main__':
    ...
    algorithm.min_delta_hv = 0.001  # 0.1
    algorithm.multiprocess = False
    algorithm.what_if = True
    ...
    algorithm.early_termination = {'max_iters': 0, 'max_duration': 240,
                                   'max_no_eliminated': 10000}  # max_duration:运行时长，按小时计
    if is_referred:
    ...
    save_path = rf"../data/output/mosa/250630/{city_name}_250630"
