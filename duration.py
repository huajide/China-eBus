...
def start_end_point(multilinestring):
    ...
    first_point = first_line.coords[0]  # 第一个点
...
def check_stops(stops):
    ...
    if 'lon' not in stops.columns or 'lat' not in stops.columns:
        print("未找到 lon 和 lat 字段，正在生成（基于 EPSG:4326）...")
        # 转换为 EPSG:4326 以提取经纬度
...
def get_all_pt_duration(timetable,stops, CITY, DATE, KEY):
    ...
    """步行小于200米且无换乘，说明识别成功。否则凑活用"""
    """爬下来的全保存，时间还是照样区分"""
    """对于环线、有地铁直达的情况，后期通过平均速度检查"""
...
def reduce_duration(row):
    ...
    # if min_itv != 60 or max_itv != 120:   # according to the assume_timetable() in timetable.py
    #
    #
    #     if min_itv > 40 and row['distance'] < 3:
    #         row['duration_peak'] = -1
    #         row['duration_offpeak'] = -1
    #     else:
    #         delta_reduction = min(min(30, min_itv/2) * 60, 0.3 * row['duration_peak'],  0.3 * row['duration_offpeak'])
    #         row['duration_peak'] = row['duration_peak'] - delta_reduction
    #         row['duration_offpeak'] = row['duration_offpeak'] - delta_reduction
    #         if row['distance']/row['duration_peak']*3600 < 5:
    #             row['duration_peak'] = row['distance'] / 5 * 3600
    #         if row['distance']/row['duration_offpeak']*3600 < 5:
    #             row['duration_offpeak'] = row['distance'] / 5 * 3600
    # else:
    #     row['duration_peak'] = -1
    #     row['duration_offpeak'] = -1
...
def fill_peak_and_offpeak_blank(row):
    if row['duration_offpeak'] <= 0 and row['duration_peak'] <= 0:
        stop_count = row['route_stop'].count(',')
        dur = row['distance'] / 50 * 3600 + stop_count*60  # 假设停一站会遇到一个路口；每停一次多30秒
...
def update_durations(routes):
    """
    除了填补空白，还因为duration包含了等车时间，应该扣除发车间隔的1/2
    :param routes: timetable after the duration scawling
